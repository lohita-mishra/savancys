...
@Component(
	...
	property = {
		...
		"javax.portlet.display-name=RfnexxContactUsWeb",
...
public class RfnexxContactUsWebPortlet extends MVCPortlet {
	...
	private void saveRfnexxContact(ResourceRequest req, ResourceResponse resp) throws PortalException {
		log.info("4 saveRfnexxContact :: " + ContactUsLocalServiceUtil.getContactUsesCount());
		...
		if((!InputValidator.isValidText(fullname)) || !InputValidator.isValidEmail(email)){
			SessionErrors.add((PortletRequest) req, "invalid-name");
			SessionErrors.add((PortletRequest) req, "invalid-email");
		}else{	
			
			ContactUsLocalServiceUtil.saveSapnexxContact(fullname, "", email, "", additionalInfo, themeDisplay);
